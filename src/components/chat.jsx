...
const socket = io('http://localhost:4000'); // establish socket connection
...
function Chat() {
    ...
    function logOut(){ // logout 
        ...
        socket.emit('unregister', localStorage.getItem('username')) // remove the user socketID from the backend
    ...
    useEffect(()=>{ // resetting input values when start adding or making group 
        setInputValue("")
    ...
    function handleImageChange(e){ // for uploading images
    ...
    useEffect(()=>{  // for changing profile photo
        if(uploading_data){
            set_updated_profile_photo(null);
            ...
            axios.post('http://localhost:4000/edit-profile', formData,  {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}` // JWT token verification
              }
            })
            .then((response)=>{
